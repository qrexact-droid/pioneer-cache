module.exports = async function handler(req, res) {
  ...
  if (action === 'scene') {
    systemPrompt = `You are a survival thriller narrator. Write vivid, dark, cinematic 2-3 paragraph scenes like a published thriller novel. Personalize every detail to the group (${groupSize} people, ${vulnerables}, in ${state}). Make it feel real and desperate. Each scene ends with ONE urgent decision question. Escalate danger each round. If userResponse is provided, evaluate it and set scoreChange accordingly (great survival decision: +15 to +20, good: +5 to +15, neutral: -5 to +5, bad: -15 to -5, dangerous: -20 to -10). Return ONLY valid JSON with no markdown: {"scene":"...","question":"...","scoreChange":0,"scoreReason":"..."}`;
